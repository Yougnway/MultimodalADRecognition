...
# get dataloader
train_loader = get_audioloader('nfoldsplits/train_'+str(FOLD_N)+'.csv')
val_loader = get_audioloader('nfoldsplits/val_'+str(FOLD_N)+'.csv', shuffle=False)
...
model = WavClassifier()
# model.load_state_dict(torch.load('save_models/audio_'+str(seed_val)+'/audio-'+str(FOLD_N)+'.pth'))
# print(model)
...
for epoch in range(EPOCHES):
    ...
    print("  Accuracy: {:.2f}\t"
          "  precision: {:.2f}\t"
          "  f1 score: {:.2f}".format(accuracy_score(y_true, y_pred),
...
# print("Saving model to: ", 'save_models/'+SPLIT+'/audio/audio-'+str(FOLD_N)+'.pth')
# torch.save(model.module.state_dict(), 'save_models/'+SPLIT+'/audio/audio-'+str(FOLD_N)+'.pth')
